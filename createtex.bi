...
private function CreateTexture( byval buffer as any ptr, byval flags as integer = 0 ) as GLuint
  ...
  dim as ulong col
	dim tex as GLuint
	dim as GLenum format2, minfilter, magfilter
	...
	if( flags and TEX_MIPMAP) then
		gluBuild2DMipmaps GL_TEXTURE_2D, format2, w, h, GL_RGBA, _
		...
		glTexImage2D GL_TEXTURE_2D, 0, format2, w, h, 0, GL_RGBA, _
			GL_UNSIGNED_BYTE, @dat(0)
		minfilter = magfilter
	end if
